  ...
  const highlightPathToRoot = (node) => {
    ...
    highlightedNodes.forEach((highlightedNode) => {
      highlightedNode.animate({
        style: {
          ...
          'color': '#304455' // revert to original node text color
        },
    ...
    highlightedEdges.forEach((highlightedEdge) => {
      highlightedEdge.animate({
        style: {
          'line-color': '#ff82a9',
          'color': '#241E92' // revert to original edge text color
        },
        duration: 2000 // transition duration
      });
    });
  ...
  useEffect(() => {
    ...
    const cyInstance = cytoscape({
      ...
      layout: {
        name: "dagre",
        rankDir: "TD",
        rankSep: 500, // increase this value for more vertical space between nodes
